        ...
        <Stack spacing={1.5} sx={{ pr: 12 }}>
          ...
          >
            ...
            <Divider
              ...
              flexItem
              sx={{ height: 20, alignSelf: "center", opacity: 0.5 }}
            />
...
function NotesCell({ order, onSave, onNotify }) {
  ...
  return (
    <Tooltip
      title={order.fulfillmentNotes || "Click to add note"}
      arrow
      placement="left"
    >
      <Box
...
export default function FulfillmentNotesPage() {
  ...
  const handleSaveNote = async (orderId, newNote) => {
    await api.patch(`/ebay/orders/${orderId}/fulfillment-notes`, {
      fulfillmentNotes: newNote,
    });
    // Refresh orders to show updated note
    setOrders((prev) =>
      prev.map((o) =>
        o._id === orderId ? { ...o, fulfillmentNotes: newNote } : o
      )
    );
  };
                    ...
                    <TableRow key={order._id || idx} hover>
                      ...
                      <TableCell>
                        <Chip
                          ...
                          variant="outlined"
                          color={
                            order.purchaseMarketplaceId === "EBAY_US"
                              ? "primary"
                              : order.purchaseMarketplaceId === "EBAY_CA" ||
                                order.purchaseMarketplaceId === "EBAY_ENCA"
                              ? "secondary"
                              : order.purchaseMarketplaceId === "EBAY_AU"
                              ? "success"
                              : "default"
                          }
                        />
